---
title: "What we're sketching, and why we're asking before we build"
date: 2026-04-27
description: "An introduction from the team scoping AEWV Compliance — a tool for accredited employers, around Post-Accreditation Checks, the 2026 National Occupation List, and MSD engagement obligations. Nothing's built yet. We're asking employers what's hard."
poll:
  question: "Of these, which carries the most weight at your place right now?"
  options:
    - "Keeping records current as roles and people shift"
    - "Producing wage and hours evidence when an audit lands"
    - "Watching for drift — wages, hours, role boundaries"
    - "Something else entirely (we'd want to know what)"
---

This is the opening note from the team scoping AEWV Compliance — a tool we're sketching for New Zealand accredited employers, around Post-Accreditation Checks, the 2026 NOL framework, and MSD engagement obligations. Nothing's built. We're talking to accredited employers first.

The reason this work exists at all: the consequences of getting AEWV compliance wrong have escalated. A Post-Accreditation Check that surfaces a gap can produce an infringement notice, the loss of accreditation, and placement on the INZ Stand-down List. For most accredited employers, that's a material business event — not a paperwork problem.

## The thinking we've put down so far

1. **Audit-Ready Record Keeping is the deliverable.** If we build it, the job would be to produce a defensible record at the moment INZ asks for it — not to help you reconstruct one.
2. **Evidence captured at the point of decision.** Role mapping against the 2026 NOL, settlement information delivery, MSD engagement, wage threshold checks — each timestamped where it happens, not pieced together later.
3. **Drift is a risk, not an inconvenience.** Wage thresholds, hours, and role boundaries would need to be monitored continuously, so a slow drift never becomes an infringement notice.

That's the shape. We don't know yet whether it matches the load you actually carry, or whether the gap is somewhere else entirely. That's what we'd value hearing about.

If your organisation holds AEWV accreditation, the [survey]({{< ref "/" >}}) is the fastest way to tell us where we've got it right and where we haven't.

{{< callout type="warning" >}}
**On the nature of this writing.** The notes published here are general commentary on the AEWV regime and accredited employer obligations. They are not legal, immigration, or compliance advice. For decisions affecting your organisation, engage a licensed immigration adviser or a NZ-qualified lawyer.
{{< /callout >}}
